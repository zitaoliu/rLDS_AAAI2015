function [bestA, bestC, bestQ, bestR, bestInitx, bestInitV, LL] = ...
    learn_kalman_rank(data, A, C, Q, R, initx, initV, lambdaA, lambda2A, ...
    sdgMaxIter, max_iter, diagQ, diagR, ARmode, constr_fun, varargin)
% LEARN_KALMAN_RANK Find the ML parameters of a stochastic Linear Dynamical System using EM with Nuclear Norm regularization.
%
% [bestA, bestC, bestQ, bestR, bestInitx, bestInitV, LL] = learn_kalman_rank(data, A, C, Q, R, initx, initV, lambdaA, lambda2A, sdgMaxIter) fits
...
%
% lambdaA is the penalty parameter for nuclear norm
% lambda2A is the penalty parameter for frobenius norm
% sdgMaxIter is the number of iterations for Singular Value Thresholding algorithm
%
...
% learn_kalman_rank(data, A, C, Q, R, initx, initV, lambdaA, lambda2A, sdgMaxIter, max_iter, diagQ, diagR, ARmode, constr_fun, varargin)
...
while ~converged && (num_iter <= max_iter)
    ...
    if loglik > maxLoglik && num_iter > 1
        bestA = A;
        bestC = C;
        bestQ = Q;
        bestR = R;
    ...
    invQ = psdInverse(QEigVecs, QEigVals);
    stepSize = 1./(norm(invQ, 'fro') * norm(gamma1, 'fro') + lambda2A);
    [A, ~] = istaANorm( A, sdgMaxIter, gamma1, beta, stepSize, thresh, lambdaA, Q, lambda2A );
    
    if sum(sum(abs(A))) < 1e-3
        break;
    end
